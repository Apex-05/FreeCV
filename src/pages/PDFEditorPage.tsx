 ...
 * PDF Direct Editor.
 *
 * PDF.js renders each page to an offscreen canvas, shown as the background image.
 * Transparent divs sit on top of every text item: clicking one covers the original
 * text with a white rectangle and swaps in a contentEditable div using font/size/
 * color sampled from the PDF. Export rebuilds that composite in a hidden print DOM
 * and calls window.print(), so unedited graphics survive untouched.
 */
...
// Items containing pictographic characters get a static overlay (no editing).
 ...
 * Measures the region's average brightness, then averages the pixels that
 * contrast with it (dark ones on a light background, bright ones on a dark one).
 */
...
/**
 * Bold glyphs lay down more ink per unit area. Sample the middle 40% of the
 * character height and count "ink" pixels: regular lands around 18-22%, bold
 * around 28-35%, so 0.27 separates them on standard black-on-white resumes.
 */
...
/**
 * The overlay div is always mounted (avoids conditional-render ref timing bugs);
 * contentEditable and text color toggle via state. Transparent when idle so the
 * PDF canvas shows through, blue-tinted on hover, opaque over a white cover
 * while editing.
 */
...
const PDFTextSpan: React.FC<TextItemProps> = ({ item, currentText, isEdited, onChange }) => {
  ...
  // Emoji / non-editable: PDF canvas shows it, so no overlay needed
...
export const PDFEditorPage: React.FC = () => {
  ...
  /**
   * Build a hidden print DOM (each page is the original canvas <img> plus
   * white-cover and new-text divs for edited items), then print it.
   */
