...
export const HipsterTemplate: React.FC<Props> = ({ data, isPrinting }) => {
  ...
  const SkillTagInput = ({ existing, onAdd }: { existing: string[]; onAdd: (tags: string[]) => void }) => {
    ...
    const commit = () => {
      ...
      if (trimmed && !existing.includes(trimmed)) onAdd([...existing, trimmed]);
      setVal('');
    };
    return (
      <input
        ...
        onBlur={commit}
        placeholder="Add tag… (Tab)"
        style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 999, color: '#fff', fontSize: fs - 2 + 'px', padding: '2px 8px', outline: 'none', width: 90, marginBottom: 4 }}
      />
    );
  };
  ...
  return (
    <div style={{ fontFamily: settings.fontFamily + ', "Nunito", sans-serif', fontSize: fs + 'px', lineHeight: settings.lineHeight, display: 'flex', background: '#fff', color: '#2d3748' }}>
              ...
              return (
                <div key={sk.id} className="group" style={{ marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
                    {tags.map((tag, i) => (
                      <span key={i} className="group/tag" style={{ display: 'inline-flex', alignItems: 'center', background: 'rgba(255,255,255,0.2)', color: '#fff', fontSize: fs - 2 + 'px', padding: '2px 8px', borderRadius: 999, marginRight: 4, marginBottom: 4, gap: 4 }}>
                        {tag}
                        {!isPrinting && (
                          <button onClick={() => store.updateSkillCategory(sk.id, 'skills', tags.filter((_, j) => j !== i).join(', '))} className="no-print" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.5)', padding: 0, lineHeight: 1, fontSize: 10 }}>×</button>
                        )}
                      </span>
                    ))}
                    {!isPrinting && <SkillTagInput existing={tags} onAdd={newTags => store.updateSkillCategory(sk.id, 'skills', newTags.join(', '))} />}
                  </div>
                </div>
              );
            })}
            <Add onClick={() => store.addSkillCategory()} label="+ Add skill group" light />
          </div>
        )}
      ...
      {/* ── Right Main — ordered by data.sections, excluding sidebar-fixed items ── */}
