    ...
    <div style={{ fontFamily: settings.fontFamily + ', "Palatino Linotype", Georgia, serif', fontSize: fs + 'px', lineHeight: settings.lineHeight, padding: `${settings.topMargin * 4 + 10}px 44px 48px`, color: '#1a1a1a', background: '#fff' }}>
      ...
      {visible('summary') && (
        <div style={gap}>
          ...
          <Row>
            <EditableField value={data.summary} onChange={v => store.updateSummary(v)} multiline style={{ color: '#374151', display: 'block', lineHeight: '1.7' }} />
          </Row>
        </div>
      )}
            ...
            <Row key={edu.id} date={dateStr(edu.startDate, edu.endDate)}>
              <div className="group">
                ...
                {edu.bullets.map((b, bi) => (
                  <EditableBullet key={bi} value={b} accentColor={acc} onChange={v => store.updateEducationBullet(edu.id, bi, v)} onRemove={() => store.removeEducationBullet(edu.id, bi)} isPrinting={isPrinting} />
                ))}
              </div>
            </Row>
          ))}
      ...
      {/* ── Experience (Research / Academic Positions) ── */}
      {visible('experience') && (
        <div style={gap}>
