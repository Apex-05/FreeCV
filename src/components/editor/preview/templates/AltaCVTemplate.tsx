...
export const AltaCVTemplate: React.FC<Props> = ({ data, isPrinting }) => {
  ...
  // Skill with dot rating
  const SkillDots = ({ level = 3, max = 5 }: { level?: number; max?: number }) => (
    <div style={{ display: 'flex', gap: 3, marginLeft: 6 }}>
      {Array.from({ length: max }).map((_, i) => (
        <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: i < level ? acc : acc + '30' }} />
      ))}
    </div>
  );
    ...
    <div style={{ fontFamily: settings.fontFamily + ', sans-serif', fontSize: fs + 'px', lineHeight: settings.lineHeight, background: '#fff', color: '#2d3748' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch' }}>
        ...
        <div style={{ width: '62%', padding: '16px 20px 40px 32px', borderRight: `1px solid #e2e8f0` }}>
          ...
          {data.customSections.filter(cs => visible(cs.id)).slice(0, Math.ceil(data.customSections.length / 2)).map(cs => (
        ...
        <div style={{ flex: 1, padding: '16px 24px 40px 20px' }}>
                ...
                <div key={sk.id} className="group" style={{ marginBottom: 6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                      ...
                      <SkillDots level={Math.min(5, Math.max(2, 5 - (i % 3)))} />
                    </div>
          ...
          {data.customSections.filter(cs => visible(cs.id)).slice(Math.ceil(data.customSections.length / 2)).map(cs => (
            <div key={cs.id} style={{ marginBottom: 16 }}>
              <CustomSectionBlock section={cs} accentColor={acc} fontSize={fs} lineHeight={settings.lineHeight} sectionSpacing={settings.sectionSpacing} isPrinting={isPrinting} headerElement={<LSH label={cs.title} />} />
            </div>
          ))}
        </div>
      </div>
